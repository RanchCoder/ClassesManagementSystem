<%@page import="com.ProjectClassFiles.*"%>
 ...
 <%
   int identity = Integer.parseInt(request.getParameter("id"));
	...
	Tution t = new Tution();
	 t = TutionWorker.getBatchById(identity);
  %>
  <html>
 ...
 <style> 
   ...
   <body onload="getData()">
		...
		<nav class="navbar navbar-inverse">
         ...
         <h2>WELCOME...</h2>			   
		  ...
		  <div class="col-sm-6">
             ...
             <h1>BATCH BY ID</h1> <br />





       <form method="post" action="updateOrDeleteBatch.jsp">
	           

			   
		 <div class="form-group">
		   <label for="id"> Id</label>
		   <input type="number" required name="Bid" id="IDE" class="form-control"> 
		   </div>
		   ...
		   <div class="form-group">
			...
			<input type="text" pattern="[A-Za-z]{,25}" requred title="must contain only characters" name="Bname" id="name" class="form-control">
            </div>

			 <div class="form-group">
			<label  for="time"> TIMING</label>
            <input type="text" required name="Btime" id="time" class="form-control">
             </div>
             <div class="form-group">
			 <label  for="subject"> Subject</label>
             <input type="text" pattern="[A-Za-z]{25}" requred title="must contain only characters" name="Bsubject" id="subject" class="form-control">
              </div>
	  ...
	  function getData(){
	      ...
	      var a =   document.getElementById("name").value="<%= t.getBatchName()%>";
			 document.getElementById("IDE").value="<%= t.getBatchId()%>";
			 document.getElementById("time").value="<%= t.getBatchTiming()%>";
              document.getElementById("subject").value="<%= t.getBatchSubject()%>";
