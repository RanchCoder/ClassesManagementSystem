...
response.setHeader("Cache-control","no-cache, no-store, must-revalidate");
   ...
   int identity = Integer.parseInt(request.getParameter("id"));
	...
	Tution t = new Tution();
	 t = TutionWorker.getManagerById(identity);
  %>
  <html>
 ...
 <style> 
   ...
   <body onload="getData()">
       ...
       <form method="post" action="updateOrDeleteManager.jsp">
	    <div class="form-group">
		   <label for="id"> Id</label>
		   <input type="number" name="Mid" id="IDE" class="form-control" required> 
		   </div>
		   ...
		   <div class="form-group">
			...
			<input type="text" pattern="[A-Za-z]{,25}" onkeyup="lettersOnly(this)" requred title="must contain only characters" name="Mname" id="name" class="form-control" required>
            </div>

			 <div class="form-group">
			<label  for="batchId">Email</label>
            <input type="email" name="MuserId" id="userId" class="form-control" required>
             </div>
    ...
    <div class="col-sm-3"></div> 
      ...
      <script>
	  function getData(){
	         document.getElementById("name").value="<%= t.getManagerName()%>";
			 document.getElementById("IDE").value="<%= t.getManagerId()%>";
			 document.getElementById("userId").value="<%= t.getManagerUserId()%>";
