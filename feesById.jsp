...
response.setHeader("Cache-control","no-cache, no-store, must-revalidate");
   ...
   int identity = Integer.parseInt(request.getParameter("id"));
	...
	Tution t = new Tution();
	 t = TutionWorker.getFeesById(identity);
  %>
  <html>
   ...
   <body onload="getData()">
		...
		<div class="row">
		  ...
		  <div class="col-sm-6">
             ...
             <h1>FEES BY ID</h1> <br /><br />



       <form method="post" action="updateOrDeleteFees.jsp">
	    <div class="form-group">
		   <label for="IDE"> Id</label>
		   <input type="id"  name="Fid" id="IDE" class="form-control" required> 
           </div>
		   
		   <div class="form-group">
			<label  for="amount"> Amount</label>
			<input type="text" pattern="[A-Za-z]{25}" requred title="must contain only characters" name="Famount" id="amount" class="form-control" required>
            </div>

			 <div class="form-group">
			<label  for="peding">Peding</label>
            <input type="number" name="Fpending" id="pending" class="form-control" required>
             </div>
              
		   
			 <div class="form-group">
			<label  for="paid">Paid</label>
            <input type="number" name="Fpaid" id="paid" class="form-control" required>
             </div><input type="submit" value="update" name="action"> &nbsp;&nbsp;&nbsp;<input type="submit" value="update" name="action">
	   </form>
    ...
    <div class="col-sm-3"></div> 
      ...
      <script>
	  function getData(){
		   document.getElementById("IDE").value="<%= t.getFeesId()%>";
	         document.getElementById("amount").value="<%= t.getFeesAmount()%>";
			 document.getElementById("pending").value="<%= t.getFeesAmountPending()%>";
			 document.getElementById("paid").value="<%= t.getFeesAmountPaid()%>";
