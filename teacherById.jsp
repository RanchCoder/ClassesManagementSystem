...
response.setHeader("Cache-control","no-cache, no-store, must-revalidate");
   ...
   int identity = Integer.parseInt(request.getParameter("id"));
	...
	Tution t = new Tution();
	 t = TutionWorker.getTeacherById(identity);
  %>
  <html>
  ...
  <style> 
   ...
   <body onload="getData()">
		...
		<nav class="navbar navbar-inverse">
		 ...
		 <div class="row">
		  ...
		  <div class="col-sm-6">
             ...
             <h1>Teacher BY ID</h1> <br /><br />


     
       <form method="post" action="updateOrDeleteTeacher.jsp">
	    <div class="form-group">
		   <label for="IDE"> Id</label>
		   <input type="number" name="Tid" id="IDE" class="form-control" required> 
		   </div>
		   ...
		   <div class="form-group">
			...
			<input type="text" pattern="[A-Za-z]{,25}" onkeyup="lettersOnly(this)" requred title="must contain only characters" name="Tname" id="name" class="form-control" required>
            </div>

			 <div class="form-group">
			<label  for="batchId">batch</label>
            <input type="number" name="TbatchId" id="batch_id" class="form-control" required>
             </div>
    ...
    <div class="col-sm-3"></div> 
      ...
      <script>
	  function getData(){
	         document.getElementById("name").value="<%= t.getTeacherName()%>";
			 document.getElementById("IDE").value="<%= t.getTeacherId()%>";
			 document.getElementById("batch_id").value="<%= t.getTeacherBatchId()%>";
